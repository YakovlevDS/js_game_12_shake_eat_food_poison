...
function getRandomItemMild(max=mild.length){
	return mild[Math.floor(Math.random() * max )]
} 
function getRandomItemPois(max=poison.length){
	return poison[Math.floor(Math.random() * max )]
} 
...
class Food {
	...
	draw() {
		...
		text(getRandomItemMild(mild.length), this.x - 3, this.y + this.size - 2);
...
class Poison {
	...
	draw() {
		...
		ellipseMode(CORNER);
		// circle(this.x, this.y, this.size);
		text(getRandomItemPois(poison.length), this.x - 3, this.y + this.size - 2);
	}
}
